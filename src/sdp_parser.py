...
from exceptions import SdpParserError


def parse_version(value):
    if value != "0":
        raise SdpParserError(
...
def parse_origin(value):
    ...
    if len(subfields) != 6:
        raise SdpParserError("Unexpected format found while parsing origin header (o=)")
...
def parse_media(value):
    ...
    try:
        ...
        raise SdpParserError(f"Invalid media description's port sub-field found: <{port_str}>")
